...
vi.mock("@prisma/client", () => {
	const mock = {
		transitCategory: { findUnique: vi.fn() },
		transitService: {
			findMany: vi.fn(),
			findFirst: vi.fn(),
			create: vi.fn(),
...
describe("api/transit_page/[category] handlers", () => {
	beforeEach(() => {
		vi.resetModules();
		mockResponseJson();
	...
	it("GET 404 when category missing", async () => {
		...
		const { GET } = await import("../../src/app/api/transit_page/[category]/route");
		const res: any = await GET(makeReq(), contextWith("bus"));
		expect(res.status).toBe(404);
	});

	it("GET returns services", async () => {
		const prismaModule = await import("@prisma/client");
		(prismaModule as any).__mock.transitCategory.findUnique = vi.fn().mockResolvedValue({ id: 1 });
		(prismaModule as any).__mock.transitService.findMany = vi.fn().mockResolvedValue([{ id: 1 }]);
		const { GET } = await import("../../src/app/api/transit_page/[category]/route");
		const res: any = await GET(makeReq(), contextWith("bus"));
